...
C2SStream.prototype.onRawStanza = function(stanza) {
	...
	else if (stanza.is('iq') && stanza.getChild('query', NS_PRIVACY)){
	...
	if (stanza.is('iq') &&
		 ...
		 stanza.getChild('session', NS_SESSION)) {
			...
			this.emit('check-privacy', stanza, function(error){
				if(!error){
	...
	if (stanza.attrs.type === 'get') {
		...
		if (privacy.getChild('list')) {
			self.emit('retrieve_privacy', {jid: stanza.attrs.from.split("/")[0], list:'default'}, function(privacy_list){
				reply.c("list", {name: "default"});
				for(i=0; i<privacy_list.length; i++){
					console.log(i);
					reply.c('item', {"type": privacy_list[i].type, "value":privacy_list[i].value, "action":privacy_list[i].action}).up();
				}
				self.send(reply);
			});
		}
