...
C2SServer.prototype.acceptConnection = function(socket) {
    ...
    socket.addListener('error', function() { 
		//console.log("In c2s : socket error");
	});
    client.addListener('error', function() { 
		//console.log("In c2s : client error"); 
	});
};
...
C2SStream.prototype.sendFeatures = function() {
    ...
    if (!this.authenticated) {
		...
		//console.log("In startStrem, auth mechanisms available: ");
		//console.log(this.mechanisms[0].name);
...
C2SStream.prototype.onRawStanza = function(stanza) {
	...
	if (stanza.is('iq') && stanza.getChild('query', NS_PRIVACY)){
		...
		if (stanza.is('message')){
			this.emit('check-privacy', {from: new JID(stanza.attrs.from).bare().toString(), to: new JID(stanza.attrs.to).bare().toString()}, function(error){
				if(!error){
					try{
						messageBody = JSON.parse(stanza.getChildText('body'));
						if (messageBody.type.valueOf() === 'game'){
							if(messageBody.action.valueOf() === 'init'){
								self.emit('game-init', stanza);
							}
							else if(messageBody.action.valueOf() === 'accept'){
								self.emit('game-accept', stanza);
							}
							else if(messageBody.action.valueOf() === 'reject'){
                                self.emit('game-reject', stanza);
                            }
							else if(messageBody.action.valueOf() === 'start'){
                                self.emit('game-start', stanza);
                            }
							else if(messageBody.action.valueOf() === 'turn'){
                                self.emit('game-turn', stanza);
                            }
							else if(messageBody.action.valueOf() === 'end'){
                                self.emit('game-end', stanza);
                            }
						}
					}
					catch(e){
...
C2SStream.prototype.authenticate = function(username, password) {
	...
	//Some one should probably catch 'authenticate', interact with database and act accordingly
...
C2SStream.prototype.onAuth = function(stanza) {
	...
	//console.log(matchingMechs[0].name);

    if (matchingMechs[0]) {
		//console.log("selected mechanism : ");
        this.mechanism = matchingMechs[0];
		//console.log(this.mechanism.name);
...
C2SStream.prototype.onPrivacy = function(stanza){
	var self = this;
	...
	if (stanza.attrs.type === 'get') {
		...
		if (privacy.getChild('list')) {
			self.emit('retrieve_privacy', {jid: stanza.attrs.from.split("/")[0], list:'default'}, function(privacy_list){
				reply.getChild('query').c("list", {name: "default"});
				for(i=0; i<privacy_list.length; i++){
					console.log(i);
		...
		console.log(1);
		console.log(privacy.getChild('list').getChild('item'));
		console.log(2);
		self.emit('set-privacy', {jid: stanza.attrs.from.split("/")[0], list:'default', item:privacy.getChild('list').getChild('item')}, function(error){
			if (!error){
				self.send(reply);
			}
		});
