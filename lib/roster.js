...
Roster.prototype.add = function(jid, callback) {
    ...
    self.itemForJid(jid, function(contact) {
		...
		rosterCollection.update({'key' : Roster.key(self.owner)}, {'$addToSet' : {'contactJIDs' : contact.jid}}, {safe : true}, function(err, result){
			callback(contact);
		});
    });
}
...
RosterContact.prototype.save = function(callback) {
    var self = this;
	contactCollection.update({'key':RosterContact.key(self.roster.owner, self.jid)}, {'state':self.state, 'name':self.name}, {upsert:true, safe:true}, function(err, result){
		assert.equal(err, null);
...
RosterContact.find = function(roster, jid, cb) {
	var self = this;
	contactCollection.findOne({key: RosterContact.key(roster.owner, jid)}, {contactInfo:1}, function(err, doc){
		assert.equal(err, null);
		if(doc == null) {
