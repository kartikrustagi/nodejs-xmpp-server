 ...
 * sessionId = {activeUsers : valArray, gameId : val, states : valArray, activeUserCount : valArray}
*/
...
GameActiveSession.prototype.saveSession = function(callback){
	gameActiveSessionCollection.update({'sessionId':this.sessionId}, {'sessionId':this.sessionId, 'activeUsers':this.activeUsers, 'gameId':this.gameId, 'states':this.states, 'activeUserCount':this.activeUserCount}, {upsert:true, safe:true}, function(err, result){
		assert.equal(err, null);
		callback(err);
	});
}
