...
User.prototype.delete = function(callback) {
    var self = this;
    client.del(this.jid.bare().toString(), function(err, obj) {
        callback(err, self);
    });
};
*/
...
User.prototype.save = function(callback) {
    var self = this;
	users.save({'jidStr' : self.jid.bare().toString(), 'password' : self.attrs.password}, function (err, res) {
		callback();
	});
};
...
User.register = function(jid, password, options) {
	users.findOne({'jidStr' : jid.bare().toString()}, function (err,doc){
		if (err || doc){
