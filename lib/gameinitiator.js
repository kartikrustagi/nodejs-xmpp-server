...
Gameinitiator.prototype.createSession = function(opts, cb){
    ...
    gameinitiator.save({"_id":this.sessionId, "creator":opts["creatorJID"], "createdAt":new Date().getTime(), "pendingJIDs":opts["requestedJIDs"], "acceptedJIDs":Array({"jid":opts["creatorJID"],"acceptanceTime":new Date().getTime()}), rejectedJIDs:[]}, function(error){
        cb(error);
    });
};
