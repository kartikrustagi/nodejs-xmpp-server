...
function C2SServer(options) {
    ...
    }).listen(options.port || 5222, options.domain_custom || '::');
...
C2SServer.prototype.acceptConnection = function(socket) {
    ...
    socket.addListener('error', function() { console.log("In c2s : socket error");});
    client.addListener('error', function() { console.log("In c2s : client error"); });
};
...
function C2SStream(socket, server) {
	...
	this.domain_custom = streamAttrs.to;
...
C2SStream.prototype.startStream = function(streamAttrs) {
	console.log("C2S StartStream");
	console.log(streamAttrs);
    var attrs = {};
...
C2SStream.prototype.authenticate = function(username, password) {
    var self = this;
    var jid = new JID(username, this.domain_custom ? this.domain_custom.toString() : "");
