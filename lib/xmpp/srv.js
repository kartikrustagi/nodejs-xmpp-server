...
function resolveSrv(name, cb) {
    dns.resolveSrv(name, function(err, addrs) {
        if (err) {
            /* no SRV record, try domain_custom as A */
            cb(err);
        } else {
...
exports.connect = function(socket, services, domain_custom, defaultPort) {
    var listener = new EventEmitter();

    var tryServices = function() {
        var service = services.shift();
        if (service) {
            resolveSrv(service + '.' + domain_custom, function(error, addrs) {
                if (addrs)
            ...
            resolveHost(domain_custom, function(error, addrs) {
                if (addrs && addrs.length > 0) {
                    ...
                    listener.emit('error', error || new Error('No addresses resolved for ' + domain_custom));
                }
            });
        }
